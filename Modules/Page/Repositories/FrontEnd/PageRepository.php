...
class PageRepository
{
    ...
    public function getOurBranchesPage()
    {
        $id = isset(config('setting.other')['our_branches']) ? config('setting.other')['our_branches'] : 0;
        $page = $this->page->find($id);
        return $page;
    }

    public function getBeforeBuyPage()
    {
        $id = isset(config('setting.other')['before_buy']) ? config('setting.other')['before_buy'] : 0;
        $page = $this->page->find($id);
        return $page;
    }
